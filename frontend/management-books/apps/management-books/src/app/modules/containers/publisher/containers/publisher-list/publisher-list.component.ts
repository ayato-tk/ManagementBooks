...
export class PublisherListComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.loadpublishers(1, this.pageSize);
    this.searchSubject.pipe(debounceTime(300), distinctUntilChanged()).subscribe((term) => {
      this.searchTerm = term;
      this.pageIndex = 0;
      this.loadpublishers(1, this.pageSize, this.searchTerm);
    });
  }

  loadpublishers(page: number, pageSize: number, search: string = '') {
  ...
  openNewDialog() {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.loadingService.show();
        this.publisherService
          .createPublisher({ name: result.name })
          .pipe(finalize(() => this.loadingService.hide()))
          .subscribe({
            next: () => {
              this.loadpublishers(this.pageIndex + 1, this.pageSize);
  ...
  openEditDialog(publisher: BookChildDto) {
    const dialogRef = this.dialog.open(BookChildDialogComponent, {
      width: '400px',
      data: { id: publisher.id, name: publisher.name } as BookChildDto,
    });
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.loadingService.show();
        this.publisherService
          .updatePublisher({ id: publisher.id, name: result.name })
          .pipe(finalize(() => this.loadingService.hide()))
          .subscribe({
            next: () => {
  ...
  deletepublisher(publisher: BookChildDto) {
    this.loadingService.show();
    this.publisherService
  ...
  onPageChange(event: PageEvent) {
    this.loadpublishers(event.pageIndex + 1, event.pageSize);
  }
